...
const PropertyCard: React.FC<PropertyCardProps> = React.memo(({
  ...
  const owner = useMemo(() => {
    return property.owner || property.createdBy;
  }, [property.owner, property.createdBy]);

  const timeAgo = useMemo(() => {
     if (owner?.lastActive) {
         try {
             return formatDistanceToNow(new Date(owner.lastActive), { addSuffix: true });
         } catch (e) { return ""; }
     }
     return "";
  }, [owner?.lastActive]);
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
            <Box display="flex" alignItems="center" gap={1.5}>
                 <Box position="relative">
                    ...
                    />
                         <Tooltip title={owner?.isVerified ? "Verified User" : "User"}>
                             <VerifiedIcon 
                                color={owner?.isBoosted ? "secondary" : "primary"}
                                sx={{ 
                                    position: 'absolute', 
                                    bottom: -2, 
                                    right: -2, 
                                    ...
                                    bgcolor: 'white', 
                                    borderRadius: '50%',
                                    display: (owner?.isVerified || owner?.isBoosted) ? 'block' : 'none'
                                }} 
                            />
                         </Tooltip>
                 </Box>
