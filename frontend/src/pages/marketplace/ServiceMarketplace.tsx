...
const ServiceMarketplace: React.FC = () => {
  ...
  const serviceTypes = [
    ...
    { label: 'Furniture Rental', value: 'furniture_rental', icon: <FurnitureIcon /> },
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        const url = type === 'all' ? '/api/services' : `/api/services?type=${type}`;
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      ...
      {/* Trust Banner */}
      <Box mt={8} p={4} bgcolor="primary.main" color="white" borderRadius={3} textAlign="center">
        <Typography variant="h5" fontWeight="bold" gutterBottom>
          Verified & Reliable Services
        </Typography>
        <Typography variant="body1">
          Every partner on our platform is hand-picked and verified for quality service.
        </Typography>
      </Box>
    </Container>
  );
