...
exports.sendMessageToConversation = void 0;
const Conversation_1 = __importDefault(require("../models/Conversation"));
const Message_1 = __importDefault(require("../models/Message"));
const User_1 = __importDefault(require("../models/User"));
const emailService_1 = __importDefault(require("./emailService"));
const notificationService_1 = __importDefault(require("./notificationService"));
const sendMessageToConversation = async (params) => {
    const { conversationId, senderId, content, attachments = [] } = params;
    // 1. Fetch conversation and participants
    const conversation = await Conversation_1.default.findById(conversationId).populate('participants');
    if (!conversation) {
    ...
    const isSenderBlocked = senderObj.blockedUsers?.some((id) => id.toString() === otherParticipant?._id.toString());
    const isRecipientBlocked = otherParticipant?.blockedUsers?.some((id) => id.toString() === senderObj._id.toString());
    ...
    setImmediate(async () => {
        try {
            ...
            const [lastOtherMessage, users] = await Promise.all([
                ...
                User_1.default.find({ _id: { $in: [senderId, otherParticipant?._id] } }).select('name email averageResponseTime')
            ]);
            ...
            if (lastOtherMessage && lastOtherMessage.createdAt && senderUser) {
                ...
                await User_1.default.findByIdAndUpdate(senderId, { averageResponseTime: Math.round(newAvg) });
            }
            // Email Notification
            if (recipientUser && senderUser) {
                const propertyTitle = conversation.property ? 'a property' : 'your message';
                emailService_1.default.sendNewMessageNotification(recipientUser.email, senderUser.name, propertyTitle).catch((err) => console.error('Failed to send email notification:', err));
            }
