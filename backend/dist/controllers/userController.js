...
const updateCurrentUser = async (req, res) => {
    ...
    try {
        const { parseFormDataJSON } = await Promise.resolve().then(() => __importStar(require('../utils/formDataHelper')));
        ...
        if (dob !== undefined)
            profileFields.dob = dob ? new Date(dob) : undefined;
        if (occupation !== undefined)
            profileFields.occupation = occupation;
        if (personalLifestyle !== undefined)
            profileFields.personalLifestyle = parseFormDataJSON(personalLifestyle);
        if (isRoommateListed !== undefined)
            profileFields.isRoommateListed = String(isRoommateListed) === 'true';
        if (preferences !== undefined)
            profileFields.preferences = parseFormDataJSON(preferences);
        const authReq = req;
